...
class TaxDeductionsController < ApplicationController
  ...
  def index
    @tax_deduction = TaxDeduction.all.as_json(except: %i[created_at updated_at],
  ...
  def find_tax_deductions
    ...
    @tax_deduction = TaxDeduction.all.where(user_id: @user.id).as_json(except: %i[created_at updated_at],
                                                                       include: ['user' => { only: :full_name }])
    render json: @tax_deduction,
           status: :ok
  end
