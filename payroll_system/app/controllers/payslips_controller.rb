...
class PayslipsController < ApplicationController
  ...
  def find_payslips
    ...
    @payslip = Payslip.all.where(user_id: @user.id).as_json(except: %i[created_at updated_at],
                                                            include: ['user' => { only: :full_name }])
    render json: @payslip,
           status: :ok
  end
  ...
  def set_extra_fields
    ...
    raise 'Attendance and Salary required for the current month' unless attend && sal_attend
